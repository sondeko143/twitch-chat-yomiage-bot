...
**Architecture:** 2 リポジトリにまたがる純粋な依存近代化（挙動は不変）。前提として外部 git 依存 `vstreamer-protos`(ローカル: `c:\Users\me_my\vstreamer\vstreamer-protos`) の Rust スタブを新 tonic/prost で再生成し `v0.1.2` としてタグ・リリース。本体(`twitch-chat-yomiage-bot`)は開発中 `[patch]` でローカル protos を参照して反復し、確定時にタグ固定参照へ切替える。
...
### Repo A: `vstreamer-protos`（`c:\Users\me_my\vstreamer\vstreamer-protos`、別 git repo）
...
Expected: source がローカルパス（`(C:\Users\me_my\vstreamer\vstreamer-protos\rust\vstreamer_protos)` 等）になっている。現状 protos は 0.1.1(tonic 0.9) のままなのでビルドは通る（まだ変更前）。
...
> すべて `c:\Users\me_my\vstreamer\vstreamer-protos` で作業。別 git repo なので commit もそちらに入る。作業前に `git -C ../vstreamer-protos status` がクリーン & `main` であることを確認。
...
Run: `cd /c/Users/me_my/vstreamer/vstreamer-protos/rust && cargo build -p vstp_builder`
Expected: コンパイル成功（まだ生成は走らせない。lib 側が旧 tonic 0.9 でも builder 単体はビルド可）。

- [ ] **Step 4: コミット（protos repo 側）**

```bash
git -C /c/Users/me_my/vstreamer/vstreamer-protos add rust/vstp_builder/Cargo.toml rust/vstp_builder/src/main.rs
git -C /c/Users/me_my/vstreamer/vstreamer-protos commit -m "build: codegen を tonic-prost-build 0.14 へ更新"
```
...
Run: `cd /c/Users/me_my/vstreamer/vstreamer-protos/rust && cargo run -r -p vstp_builder`
Expected: `vstreamer_protos/src/voicerecog.rs` が更新される（diff が出る）。

- [ ] **Step 3: lib をビルドして生成物を確認**

Run: `cd /c/Users/me_my/vstreamer/vstreamer-protos/rust && cargo build -p vstreamer_protos`
...
Run: `cd /c/Users/me_my/vstreamer/vstreamer-protos/rust && cargo build`
Expected: 成功。

- [ ] **Step 5: コミット（protos repo 側、push はまだしない）**

```bash
git -C /c/Users/me_my/vstreamer/vstreamer-protos add rust/vstreamer_protos/Cargo.toml rust/vstreamer_protos/src/voicerecog.rs rust/Cargo.lock
git -C /c/Users/me_my/vstreamer/vstreamer-protos commit -m "feat: tonic 0.14/prost 0.14 で Rust スタブを再生成し 0.1.2 へ"
```
...
Run: `git -C /c/Users/me_my/vstreamer/vstreamer-protos log --oneline -3`
Expected: Task 1/2 の 2 コミットが `main` 先端にある。

- [ ] **Step 2: push**

```bash
git -C /c/Users/me_my/vstreamer/vstreamer-protos push origin main
```

- [ ] **Step 3: `v0.1.2` タグを打って push**

```bash
git -C /c/Users/me_my/vstreamer/vstreamer-protos tag -a v0.1.2 -m "v0.1.2: tonic 0.14/prost 0.14 stubs"
git -C /c/Users/me_my/vstreamer/vstreamer-protos push origin v0.1.2
```

Run（確認）: `git -C /c/Users/me_my/vstreamer/vstreamer-protos ls-remote --tags origin v0.1.2`
...
`C:\Users\me_my\.claude\projects\c--Users-me-my-vstreamer-twitch-chat-yomiage-bot\memory\reqwest-axum-hyper-modernization.md` を「解消済み（PR で reqwest0.13/axum0.8/hyper1/tonic0.14、protos v0.1.2 リリース）」に更新し、`MEMORY.md` の該当行も追従。
